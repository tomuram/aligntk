            ...
            char **envp)
{
  ...
  sprintf(moveFileName, "%s/moves.log", c.logBasename);
  moveFile = fopen(moveFileName, "w");
...
Compute (char *outputName, char *outputWarpedName, char *outputCorrelationName)
{
  ...
  for (level = startLevel; level >= c.outputLevel; --level)
...
#else
	    txd = fabs(fmod(nomThetaX[k] - thetaX + 3.0*M_PI, 2.0*M_PI) - M_PI);
	    tyd = fabs(fmod(nomThetaY[k] - thetaY + 3.0*M_PI, 2.0*M_PI) - M_PI);
	    de += txd * txd + tyd * tyd +
	      (l0 - nomL0[k]) * (l0 - nomL0[k]) +
	      (l1 - nomL1[k]) * (l1 - nomL1[k]) +
	      (l2 - nomL2[k]) * (l2 - nomL2[k]) +
      ...
      if (prop == NULL)
	{
      ...
      fprintf(moveFile, "STARTING %zd MOVES AT LEVEL %d\n", goalMoveCount,
	      level);
      ...
      while (moveCount < goalMoveCount)
	{
	  ...
	  ocy = cy;
	  rnd = drand48();
	  fprintf(moveFile, "drand48 returned %f\n", rnd);
	  ...
	  fprintf(moveFile, "MOVE %zd: %d %d %f %f --- ",
		  moveCount - 1, icx, icy, ocx - cx, ocy - cy);
	  ...
	  for (y = sy; y <= ey; ++y)
		...
		if (ixv < 0 || ixv >= mpw_minus_1 ||
		  ...
		  {
#if 0
		    fprintf(moveFile, "SKIPPING (1) %d %d %d %d\n",
			    ixv, iyv, mpw_minus_1, mph_minus_1);
#endif
		    continue;
		  }
		...
		if (MAP(prop, mpw, ixv, iyv).c == 0.0 &&
		  ...
		  {
#if 0
		    fprintf(moveFile, "SKIPPING (2) %f %f %f %f\n",
			    MAP(prop, mpw, ixv, iyv).c,
			    MAP(prop, mpw, ixv+1, iyv).c,
			    MAP(prop, mpw, ixv, iyv+1).c,
			    MAP(prop, mpw, ixv+1, iyv+1).c);
#endif
		    continue;
		  }
		...
		if (rc00 != 0.0 && rc01 != 0.0 && rc10 != 0.0 && rc11 != 0.0)
		  {
			...
			if ((crmask[iry*rcmbpl + (irx >> 3)] & (0x80 >> (irx & 7))) != 0 &&
		      ...
		      {
#if 0
			fprintf(moveFile, "SKIPPING (3) %d %d %d %d\n",
				irx, iry, rw-1, rh-1);
#endif
		      }
		  }
		...
		if (rc00 == 0.0 || rc01 == 0.0 || rc10 == 0.0 || rc11 == 0.0)
		  {
#if 0
		    fprintf(moveFile, "SKIPPING (4) %f %f %f %f\n",
			    rc00, rc01, rc10, rc11);
#endif
		    continue;
		  }
		...
		if (irx < 0 || irx >= rw-1 || iry < 0 || iry >= rh-1)
		  {
#if 0
		    fprintf(moveFile, "SKIPPING (5) %d %d %d %d\n",
			    irx, iry, rw-1, rh-1);
#endif
		    continue;
		  }
		  ...
		  {
#if 0
		    fprintf(moveFile, "USING (5) %d %d %d %d\n",
			    irx, iry, rw-1, rh-1);
#endif
		  }
	  ...
	  newDPoints = dPoints;
		...
		continue;
		  ...
		  ntxd = fabs(fmod(nomThetaX[k] - nThetaX + 3.0*M_PI, 2.0*M_PI) -
			      M_PI);
		  ntyd = fabs(fmod(nomThetaY[k] - nThetaY + 3.0*M_PI, 2.0*M_PI) -
			      M_PI);
#if 0
