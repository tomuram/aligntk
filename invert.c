/*
 * invert.c  - invert a map
 *
 *  Copyright (c) 2008-2012 National Resource for Biomedical
 *                          Supercomputing,
 ...
 *
 *  HISTORY
 *    2008-2012  Written by Greg Hood (ghood@psc.edu)
 */
...
InvertMap (MapElement *map, int nx, int ny)
{
  ...
  if (xMin >= xMax || yMin >= yMax)
    {
      fprintf(stderr, "InvertMap: no valid map blocks found!\n");
      free(inverseMap);
      return(NULL);
    }
  ...
  for (y = 0; y < nyp; ++y)
	...
	++e;
#else
	inverseMap->inverseMap[y*nxp + x].minX = 1000000000;
	inverseMap->inverseMap[y*nxp + x].maxX = -1000000000;
	inverseMap->inverseMap[y*nxp + x].minY = 1000000000;
	inverseMap->inverseMap[y*nxp + x].maxY = -1000000000;
#endif
      }
  ...
  for (;;)
      ...
      if (inverseMap->marked[y*nx+x])
	{
	  if (nTried >= (maxX - minX + 1) * (maxY - minY + 1))
	    {
#if DEBUG
	      printf("Error: invert() could not locate the proper MapElement\n");
	      printf("xvp = %f yvp = %f\n", xvp, yvp);
	      printf("ix = %d iy = %d\n", ix, iy);
	      printf("minX = %d maxX = %d minY = %d maxY = %d\n",
		     minX, maxX, minY, maxY);
		...
		float x11, float y11, float x10, float y10)
{
  ...
  if (dxh * dyp00 - dyh * dxp00 < 0.0 ||
	...
	{
	  if (det > -1.0e-10 * size)
	    fprintf(stderr, "Error: determinant of the map element is zero (%f)\n", det);
	  else
	    fprintf(stderr, "Error: determinant of the map element is negative (%f)\n", det);
	  *pu = 0.0;
	  *pv = 0.0;
	  return(0);
	}
