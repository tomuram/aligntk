...
main (int argc, char **argv)
{
  ...
  if (p == 0)
	...
	if (strcmp(argv[i], "-input") == 0)
      ...
      if (error)
	{
	  fprintf(stderr, "Usage: align8 -input file_prefix -output file_prefix\n");
      ...
      if (inputName[0] == '\0' || (outputName[0] == '\0' &&
				   outputMapName[0] == '\0' &&
				   outputGridName[0] == '\0'))
	Error("Both -input and -output (or -outputmap/-outputgrid) arguments must be specified.\n");
      if (nMaps == 0)
      ...
      for (m = 0; m < nMaps; ++m)
	{
	  ...
	  while ((de = readdir(dir)) != NULL)
	      ...
	      if (n < mapMin[m] || n > mapMax[m])
		{
		  ...
		  if ((w % mapHeader[0]) != 0 ||
		      (h % mapHeader[1]) != 0 ||
		      w / mapHeader[0] != h / mapHeader[1])
		    Error("Image size not multiple of map size for %s (%d %d %d %d)\n",
			  de->d_name, w, h, mapHeader[0], mapHeader[1]);
		  mapForward[m] = (mapHeader[2] > n);
		  if (mapFactor == 0)
		    mapFactor = w / mapHeader[0];
		  else
		    if (mapFactor != w / mapHeader[0])
      ...
      while ((de = readdir(dir)) != NULL)
	{
      ...
      params[2] = minZ;
      params[3] = maxZ;
      params[4] = nSkipSlices;
      params[5] = nMaps;
      params[6] = inputDigits;
      params[7] = mapFactor;
      params[8] = fixedSlice;
      params[9] = spacing;
      params[10] = outputVolumes;
      params[11] = constraintDigits;
      params[12] = showConstraints;
      params[13] = minIter;
  ...
  /* broadcast the info */
  if (MPI_Bcast(inputName, PATH_MAX, MPI_CHAR, 0, MPI_COMM_WORLD) !=
      MPI_SUCCESS)
    Error("Broadcast of input name failed.");
  if (MPI_Bcast(inputMapName, PATH_MAX, MPI_CHAR, 0, MPI_COMM_WORLD) !=
      MPI_SUCCESS)
    Error("Broadcast of input map name failed.");
  if (MPI_Bcast(outputName, PATH_MAX, MPI_CHAR, 0, MPI_COMM_WORLD) !=
      MPI_SUCCESS)
    Error("Broadcast of output name failed.");
  ...
  if (MPI_Bcast(sliceMagnificationsName, PATH_MAX, MPI_CHAR, 0, MPI_COMM_WORLD) !=
      MPI_SUCCESS)
    Error("Broadcast of slice magnifications name failed.");
  if (MPI_Bcast(params, 14, MPI_INT, 0, MPI_COMM_WORLD) != MPI_SUCCESS)
  ...
  minZ = params[2];
  maxZ = params[3];
  nSkipSlices = params[4];
  nMaps = params[5];
  inputDigits = params[6];
  mapFactor = params[7];
  fixedSlice = params[8];
  spacing = params[9];
  outputVolumes = params[10];
  constraintDigits = params[11];
  showConstraints = params[12];
  minIter = params[13];
  kAbsolute = fParams[0];
  kIntra = fParams[1];
  kInter = fParams[2];
  ...
  Log("w = %d h = %d minZ = %d maxZ = %d nS = %d nM = %d at %d\n",
      inputWidth, inputHeight, minZ, maxZ, nSkipSlices, nMaps, p);

  if (p != 0)
    skipSlices = (int*) malloc(nSkipSlices*sizeof(int));
  if (MPI_Bcast(skipSlices, nSkipSlices, MPI_INT, 0, MPI_COMM_WORLD) !=
      MPI_SUCCESS)
    Error("Broadcast of skipSlices failed.");

  slice = (int *) malloc((maxZ - minZ + 1) * sizeof(int));
  actualSlice = (int *) malloc((maxZ - minZ + 1) * sizeof(int));
  mnx = inputWidth / mapFactor;
  mny = inputHeight / mapFactor;
  nx = inputWidth / spacing;
  ny = inputHeight / spacing;
  nz = 0;
  skipIndex = 0;
  for (z = minZ; z <= maxZ; ++z)
    {
  ...
  if (sliceMagnificationsName[0] != '\0')
      ...
      if (p == 0)
	{
      ...
      if (MPI_Bcast(sliceMagnifications, nz, MPI_FLOAT, 0, MPI_COMM_WORLD) !=
	  MPI_SUCCESS)
	Error("Broadcast of sliceMagnifications failed.\n");
    }

  nPts = nx * ny;
  nFloats = (sizeof(Point) / sizeof(float)) * nPts;
  ...
  for (i = sz; i <= ez; ++i)
      ...
      if (inputMapName[0] != '\0')
	{
	  ...
	  mpwi = mapHeader[0];
	  mphi = mapHeader[1];
	  inputMapFactor = 1 << ((int) floor(log(((float) inputWidth) / mpwi) / log(2.0) + 0.5));
	  mpwip = mpwi + 2;
	  mphip = mphi + 2;
	  if (mx == NULL)
	    {
	  ...
	  for (y = 0; y < ny; ++y)
		...
		if (ix0 < -1 || ix0 >= mpwi ||
		    iy0 < -1 || iy0 >= mphi)
		  Error("Cannot obtain starting positions from input map %s\n", fn);
  ...
  for (m = 0; m < nMaps; ++m)
    {
      ...
      skipIndex = (mapForward[m] ? 0 : nSkipSlices-1);
      for (iz = mapMin[m]; iz <= mapMax[m]; ++iz)
	{
	  if (mapForward[m])
	  ...
	  else
	    {
	      ...
	      while (skipIndex >= 0 && skipSlices[skipIndex] > z)
		--skipIndex;
	      if (skipIndex >= 0 && skipSlices[skipIndex] == z)
		continue;
	    }
	  i = slice[z - minZ];
	  if (i < sz || i > ez)
  ...
  for (i = sz; i <= ez; ++i)
      ...
      if (j >= i)
	{
	  ...
	  nIntraTableEntries = k;
	  Log("Constructed  (k = %d)\n", k);
	  intraTables[i] = intraTable;
	}
    }
  if (p == 0)
    Log("nIntraTableEntries = %d\n", nIntraTableEntries);
  maxStepX = 0.1 * spacing;
  maxStepY = 0.1 * spacing;

  interTable = (InterTableEntry*) malloc(nMaps * (ez - sz) * nPts *
  ...
  for (iter = 0; nTimes < 10; ++iter)
      ...
      for (i = myFirstSlice; i <= myLastSlice; ++i)
	{
	  ...
	  pt = pts[i];
	  basis = energy;
	  for (k = 0; k < nIntraTableEntries; ++k)
	    {
	      it = &(intraTables[i][k]);
...
#if 0
	      if (d < 0.8 * nomD || d > 1.2 * nomD)
		{
		  /* double the spring stiffness */
		  pk = it->k;
		  it->k = 2.0 * pk;
		  energyCorrection += (it->k - pk) * (d - it->nomD) * (d - it->nomD);
		}
	      else
		if (it->k > kIntraThisSlice)
		  {
		    /* slightly weaken the spring stiffness back to
		       the target stiffness */
		    pk = it->k;
		    it->k = 0.99 * pk;
		    if (it->k < kIntraThisSlice)
		      it->k = kIntraThisSlice;
		    energyCorrection += (it->k - pk) * (d - it->nomD) * (d - it->nomD);
		  }
#endif
      ...
      if ((p & 1) == 0)
	{
	  ...
	  if (p != np-1)
	    {
	      if (MPI_Send(pts[myLastSlice], nFloats, MPI_FLOAT,
			   p+1, 0, MPI_COMM_WORLD) != MPI_SUCCESS)
		Error("Could not send positions forward (even-to-odd)\n");
	      if (MPI_Recv(pts[myLastSlice+1], nFloats, MPI_FLOAT,
	...
	{
	  /* receive, then send */
	  if (MPI_Recv(pts[myFirstSlice-1], nFloats, MPI_FLOAT,
		       p-1, 0, MPI_COMM_WORLD, &status) != MPI_SUCCESS)
	    Error("Could not receive positions forward (even-to-odd)\n");
	  if (MPI_Send(pts[myFirstSlice], nFloats, MPI_FLOAT,
		       p-1, 0, MPI_COMM_WORLD) != MPI_SUCCESS)
...
Output (int iter)
{
  ...
  if (outputName[0] != '\0')
    {
      ...
      output = (unsigned char *) malloc(w * h * sizeof(unsigned char));
      colorOutput = (unsigned char *) malloc(3 * w * h * sizeof(unsigned char));
      xCoord = (float *) malloc(w * h * sizeof(float));
      yCoord = (float *) malloc(w * h * sizeof(float));
      points = (point *) malloc(nx * ny * sizeof(point));
    }
  if (outputGridName[0] != '\0')
    grid = (unsigned char *) malloc(inputWidth * inputHeight * 3);
  ...
  if (p == 0)
	...
	{
	  if (iter < 0)
	    sprintf(fn, "%sfinal", outputName);
	  else
	    sprintf(fn, "%s%0.6d", outputName, iter);
	  if (mkdir(fn, 0755) != 0)
	    Error("Could not create directory %s\n", fn);
	}
    }
  ...
  for (i = myFirstSlice; i <= myLastSlice; ++i)
      ...
      if (outputMapName[0] != '\0')
	{
	  ...
	  my[0] = 0.5 * (2.0 * my[1] -
			    ...
			    my[2*nx2]);
	  mc[0] = 1.0;
	  mx[nx + 1] = 0.5 * (2.0 * mx[nx] -
                           mx[nx-1] +
                           2.0 * mx[nx2 + nx + 1] -
				   mx[2*nx2 + nx + 1]);
	  my[nx + 1] = 0.5 * (2.0 * my[nx] -
                           my[nx-1] +
                           2.0 * my[nx2 + nx + 1] -
				   my[2*nx2 + nx + 1]);
	  mc[nx + 1] = 1.0;
	  mx[(ny + 1) * nx2] = 0.5 * (2.0 * mx[ny * nx2] -
					     mx[(ny - 1) * nx2] +
					     2.0 * mx[(ny + 1) * nx2 + 1] -
					     mx[(ny + 1) * nx2 + 2]);
	  my[(ny + 1) * nx2] = 0.5 * (2.0 * my[ny * nx2] -
					     my[(ny - 1) * nx2] +
					     2.0 * my[(ny + 1) * nx2 + 1] -
					     mx[(ny + 1) * nx2 + 2]);
	  mc[(ny + 1) * nx2] = 1.0;
	  mx[(ny + 1) * nx2 + nx + 1] =
	    0.5 * (2.0 * mx[ny * nx2 + nx + 1] -
		   mx[(ny - 1) * nx2 + nx + 1] +
		   2.0 * mx[(ny + 1) * nx2 + nx] -
		   mx[(ny + 1) * nx2 + nx - 1]);
	  my[(ny + 1) * nx2 + nx + 1] =
	    0.5 * (2.0 * my[ny * nx2 + nx + 1] -
		   my[(ny - 1) * nx2 + nx + 1] +
		   2.0 * my[(ny + 1) * nx2 + nx] -
		   my[(ny + 1) * nx2 + nx - 1]);
      ...
      if (outputGridName[0] != '\0')
	{
	  memset(grid, 0, inputWidth * inputHeight * 3);
	  pt = pts[i];
	  for (y = 0; y < ny; ++y)
	    for (x = 0; x < nx; ++x)
	      for (dy = -1; dy <= 0; ++dy)
		...
		if (pt[y*nx+x].fx > 0.5 * CONSTRAINED ||
		    pt[y*nx+x].fy < 0.5 * CONSTRAINED)
		  {
		      ...
		      for (dx = -1; dx <= 1; ++dx)
			{
			  xp = ix + dx;
			  yp = iy + dy;
			  if (xp >= 0 && xp < inputWidth &&
			      yp >= 0 && yp < inputHeight)
			    {
			      grid[3*(yp * inputWidth + xp)] = 255;
			      grid[3*(yp * inputWidth + xp) + 1] = 255;
			      grid[3*(yp * inputWidth + xp) + 2] = 255;
			    }
			}
		  }
	      }
	  ...
	  fprintf(f, "P6\n%d %d\n%d\n", inputWidth, inputHeight, 255);
	  if (fwrite(grid, sizeof(unsigned char), 3*inputWidth*inputHeight, f) !=
	...
	continue;
      ...
      /* read in the image */
      Log("READING SLICE %d\n", actualSlice[i]);
      sprintf(fn, "%s%0.*d.pgm", inputName, inputDigits, actualSlice[i]);
      f = fopen(fn, "r");
      if (f == NULL)
	Error("Could not open input file %s\n", fn);
      if (!ReadHeader(f, &tc, &iw, &ih, &im))
	Error("Could not read header of image %s\n", fn);
      if (im != 255)
	Error("align can only currently deal with 8-bit grayscale images\n");
      if (iw != w || ih != h)
	Error("Inconsistent image size encountered in file %s\n", fn);
      if (fread(image, sizeof(unsigned char), w*h, f) != w*h)
	Error("Could not read image from file %s", fn);
      fclose(f);
	...
	for (x = 0; x < nx; ++x)
	  {
	    points[j].x = pt->x;
	    points[j].y = pt->y;
	    points[j].z = spacing * x + (spacing - 1.0) / 2.0;
	    ++pt;
	    ++j;
	  }
      ...
      csa_setnppc(cs, 30);
      Log("CSA_ADDPOINTS\n");
      csa_addpoints(cs, nx * ny, points);
      Log("CSA_CALCSPLINE\n");
      csa_calculatespline(cs);
      Log("APPROXIMATING\n");
      for (y = 0; y < h; ++y)
	for (x = 0; x < w; ++x)
	  {
	    csa_pt.x = x;
	    csa_pt.y = y;
	    csa_approximatepoint(cs, &csa_pt);
	    xCoord[y*w + x] = csa_pt.z;
	  }
      Log("DESTROYING\n");
      csa_destroy(cs);
      j = 0;
      for (y = 0; y < ny; ++y)
	for (x = 0; x < nx; ++x)
	  points[j++].z = spacing * y + (spacing - 1.0) / 2.0;
      Log("CSA_CREATE2\n");
      cs = csa_create();
      csa_setnpmin(cs, 20);
      csa_setnpmax(cs, 80);
	...
	for (x = 0; x < w; ++x)
	  {
	    ...
	    csa_pt.y = y;
	    csa_approximatepoint(cs, &csa_pt);
	    yCoord[y*w + x] = csa_pt.z;
	  }
      Log("DESTROYING\n");
      csa_destroy(cs);

      Log("PULLING\n");
      /* pull out the values of the image at that point using interpolation */
      background = 0;
      for (y = 0; y < h; ++y)
	for (x = 0; x < w; ++x)
	  {
	    ...
	    rv = r00 * (rrx - 1.0) * (rry - 1.0)
	      ...
	      - r01 * (rrx - 1.0) * rry
	      + r11 * rrx * rry;
	    output[y*w + x] = (unsigned char) rv;
	  }

      Log("WRITING\n");
      /* output the corrected slice */
      if (showConstraints)
	{
	  for (y = 0; y < h; ++y)
	    for (x = 0; x < w; ++x)
	      {
		colorOutput[(y * w + x) * 3] = output[y*w + x];
		colorOutput[(y * w + x) * 3 + 1] = output[y*w + x];
		colorOutput[(y * w + x) * 3 + 2] = output[y*w + x];
	      }
	  pt = pts[i];
	  for (y = 0; y < ny; ++y)
	    for (x = 0; x < nx; ++x)
	      {
		if (pt->fx > 0.5 * CONSTRAINED ||
		    pt->fy > 0.5 * CONSTRAINED)
		  {
		    if (pt->fx >= 0.5 * CONSTRAINED)
		      k = ((int) floor(pt->fx / CONSTRAINED + 0.5)) - 1;
		    else
		      k = ((int) floor(pt->fy / CONSTRAINED + 0.5)) - 1;
		    ix = (int) floor(pt->x + 0.5);
		    iy = (int) floor(pt->y + 0.5);
		    for (dy = -1; dy <= 1; ++dy)
		      {
			yp = iy + dy;
			for (dx = -1; dx <= 1; ++dx)
			  {
				...
				yp >= 0 && yp < h)
			      {
				colorOutput[(yp*w + xp) * 3] =
				    colors[(k % 10) + 10 * (k == 0)][0];
				colorOutput[(yp*w + xp) * 3 + 1] =
				  colors[(k % 10) + 10 * (k == 0)][1];
				colorOutput[(yp*w + xp) * 3 + 2] =
				  colors[(k % 10) + 10 * (k == 0)][2];
			      }
			  }
		      }
		  }
	  ...
	  if (iter < 0)
	    sprintf(fn, "%sfinal/%0.*d.ppm", outputName, outputDigits,
		    actualSlice[i]);
	  else
	    sprintf(fn, "%s%0.6d/%0.*d.ppm", outputName, iter, outputDigits,
		    actualSlice[i]);
	  f = fopen(fn, "w");
	  if (f == NULL)
	    Error("Could not open output file %s for writing\n", fn);
	  fprintf(f, "P6\n%d %d\n%d\n", w, h, 255);
	  if (fwrite(colorOutput, sizeof(unsigned char), 3*w*h, f) != 3*w*h)
	...
	{
	  if (iter < 0)
	    sprintf(fn, "%sfinal/%0.*d.%s", outputName, outputDigits,
		    actualSlice[i], showConstraints ? "ppm" : "pgm");
	  else
	    sprintf(fn, "%s%0.6d/%0.*d.pgm", outputName, iter, outputDigits,
		    actualSlice[i]);
	  f = fopen(fn, "w");
	  if (f == NULL)
	    Error("Could not open output file %s for writing\n", fn);
	  fprintf(f, "P5\n%d %d\n%d\n", w, h, 255);
	  if (fwrite(output, sizeof(unsigned char), w*h, f) != w*h)
	    Error("Could not write to output file %s\n", fn);
	  fclose(f);
	}
    }
