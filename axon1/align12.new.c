  ...
  for (i = 0; i < nImages; ++i)
      ...
      if (initialMapsName[0] != '\0')
	{
	  ...
	  */
	  ...
	  for (level = startLevel;
	       images[i].fixed ? (level == startLevel) : (level >= endLevel);
	       --level)
	    {
	      factor = 1 << level;
		...
		threshold = ((float) factor)
		  * exp(M_LN2 * (level - endLevel) / (startLevel - endLevel))
	      ...
	      nx = (images[i].width + factor - 1) / factor + 1;
	      ny = (images[i].height + factor - 1) / factor + 1;
	      Log("Initializing initialPos[%d][%d] with size %d %d\n",
		  i, level, nx, ny);
	      initialPos = malloc(nx * ny * sizeof(Position));
		  ...
		  for (x = 0; x < nx; ++x)
		    {
		      ...
		      if (images[i].nodes[y*nx+x].x == UNSPECIFIED &&
			  images[i].nodes[y*nx+x].y == UNSPECIFIED)
			{
			  ++initialPos;
			  continue;
			}
		      ...
		      if (i == ioi && x == poix && y == poiy)
#endif
			Log("setting initial pos at level %d x = %d y = %d xv = %f yv = %f ixv = %d iyv = %d dnx = %d dny = %d dist = %f thresh = %f\n",
	  ...
	  for (y = 0; y < ny; ++y)
		...
		ry = ry00 * (rrx - 1.0) * (rry - 1.0)
		  ...
		  - ry01 * (rrx - 1.0) * rry
		  + ry11 * rrx * rry;
		Log("Setting initial map of %s at (%d,%d) to (%f, %f)\n",
		    images[i].name,
		    x, y,
		    rx/scale, ry/scale);
      ...
      else if (nConstraints[i] > 0)
	{
	  ...
	  for (y = 0; y < ny; ++y)
		...
		if (node->fx < 0.5 * CONSTRAINED)
		  {
		    node->x = rx / startFactor;
		    //		    Log("Setting initial node x pos of (%d %d) to %f\n",
		    //			x, y, node->x);
		  }
		if (node->fy < 0.5 * CONSTRAINED)
		  {
		    node->y = ry / startFactor;
		    //		    Log("Setting initial node y pos of (%d %d) to %f\n",
		    //			x, y, node->y);
		  }
	      }
	}
  ...
  for (step = 0; step < nSteps; ++step)
      ...
      for (iter = 0; ; ++iter)
	{
	  ...
	  if (outputGridName[0] != '\0' &&
	    ...
	    {
	      sprintf(gridName, "%sstep%0.2d.i%0.6d.pnm",
	  ...
	  if (iter % epochIterations == 0)
	      ...
	      if (CheckForFolds(level, iter))
		{
		  ...
		  if (outputGridName[0] != '\0')
		    {
		      sprintf(gridName, "%sfold%d.%0.3d.pnm",
	  ...
	  (outputIncremental || step == nSteps-1))
	{
	  ...
	  if (step == nSteps-1)
	    ...
	    sprintf(gridName, "%sstep%0.2d.pnm",
		    outputGridName, step);
...
RefinePositions (int prevLevel, int level)
{
  ...
  for (i = 0; i < nImages; ++i)
	...
	ConstrainNodes(i,
      ...
      if (images[i].fixed && initialMapsName[0] != '\0')
	{
	  sprintf(fn, "%s%s.map", initialMapsName, images[i].name);
	  ...
	  if (mxMin != 0 || myMin != 0)
	    Error("Initial map %s is only a partial map.\n",
		  fn);
	  scale = ((double) factor1) / (1 << mLevel);
	  node = images[i].nodes;
	  for (y = 0; y < ny1; ++y)
	    for (x = 0; x < nx1; ++x, ++node)
	      {
		if (node->x > 0.5 * UNSPECIFIED)
		  continue;
		xv = scale * x;
