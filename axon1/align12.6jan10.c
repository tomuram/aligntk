...
typedef struct Image
{
  ...
  Point *points;        /* points in the image at current level */
...
int ParseSpringSpec (char *spec, int *size, int **level, int **iter,
		     float **value);
...
main (int argc, char **argv)
{
  ...
  for (i = 0; i < nImages; ++i)
	...
	continue;
      nx = images[i].nx;
      ny = images[i].ny;
      images[i].points = (Point*) malloc(nx * ny * sizeof(Point));
      pt = images[i].points;
      if (images[i].owner != p)
	{
  ...
  for (step = 0; step < nSteps; ++step)
      ...
      for (iter = 0; ; ++iter)
	{
	  ...
	  if (iter % epochIterations == 0)
	    {
	      foldDetected = CheckForFolds(level, iter, !outputIncremental);
	      if (foldDetected)
		processWithFold = p;
	      else
		processWithFold = np;
	  ...
	  for (i = 0; i < nMaps; ++i)
	    {
	      ...
	      pts0 = images[m->image0].points;
	      pts1 = images[m->image1].points;
...
RefinePositions (int prevLevel, int level)
{
  ...
  for (i = 0; i < nImages; ++i)
	...
	continue;
      ...
      images[i].ny = ny1;
      pts = images[i].points;
      pts1 = (Point*) malloc(nx1 * ny1 * sizeof(Point));
      images[i].points = pts1;
      if (images[i].owner != p)
	{
...
int
ParseSpringSpec (char *spec, int *size,
		 int **level, int **iter, float **value)
{
  int pos;
  float v;
  int i, l;
  ...
  *value = NULL;
  ...
  for (;;)
    {
      if (spec[pos] == '\0')
	  break;
      if (sscanf(&spec[pos], "%f%n", &v, &n) <= 0)
	Error("Expected spring constant in spring specification %s\n", spec);
      pos += n;
      ++*size;
      *level = (int *) realloc(*level, (*size) * sizeof(int));
      *iter = (int *) realloc(*iter, (*size) * sizeof(int));
      *value = (float *) realloc(*value, (*size) * sizeof(float));
      (*level)[*size - 1] = 100;
      (*iter)[*size - 1] = 0;
      (*value)[*size - 1] = v;
      if (spec[pos] == '@')
	{
	  ++pos;
	  if (sscanf(&spec[pos], "%d%n", &l, &n) <= 0)
	    Error("Expected integral level after @ in spring specification %s\n", spec);
	  (*level)[(*size) - 1] = l;
	  pos += n;
	  if (spec[pos] == '[')
	    {
	      ++pos;
	      if (sscanf(&spec[pos], "%d%n", &i, &n) <= 0)
		Error("Expected integral iteration number after [ in spring specification %s\n", spec);
	      (*iter)[(*size) - 1] = i;
	      pos += n;
	      if (spec[pos] != ']')
		Error("Missing right bracket in spring specification: %s\n", spec);
	      ++pos;
	    }
	}
      if (spec[pos] == ',')
  ...
  if (*size == 0)
    Error("Spring specification cannot be empty.\n");
}
