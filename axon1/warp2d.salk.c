...
/* unsigned char *folds[MAX_LEVELS]; */
  ...
  if (logFile == NULL)
    {
      ...
      i = par_instance();
      if (i < 0)
	sprintf(logName, "%s00.log", c.logDir);
      else if (i < 100)
	sprintf(logName, "%s%0.2d.log", c.logDir, i);
      else
	sprintf(logName, "%s%d.log", c.logDir, i);
