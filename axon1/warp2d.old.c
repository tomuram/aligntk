...
Compute ()
{
  ...
  for (level = nLevels - 1; level >= c.outputLevel; --level)
    {
      Log("Considering level %d\n", level);
      /* create initial mapping from next higher level mapping */
      image = images[level];
      ref = refs[level];
      ...
      else
	{
      ...
      /* force the map if an input map was given */
      if (inputMapFactor != 0)
	for (y = -1; y <= mph; ++y)
	  for (x = -1; x <= mpw; ++x)
	    {
	      ...
	      rrx = x0 - ix0;
	      rry = y0 - iy0;
	      if (ix0 < -1 || ix0 > mpwi || ix0 == mpwi && rrx > 0.0 ||
		  iy0 < -1 || iy0 > mphi || iy0 == mphi && rry > 0.0)
		{
		  rc = 0.0;
		  rx = 0.0;
		  ry = 0.0;
		...
		{
		  rx00 = MAPIN(inputMapX, ix0, iy0);
		  ry00 = MAPIN(inputMapY, ix0, iy0);
		  rc00 = MAPIN(inputMapC, ix0, iy0);
		  if (iy0 < mphi)
		    {
		      rx01 = MAPIN(inputMapX, ix0, iy0 + 1);
		      ry01 = MAPIN(inputMapY, ix0, iy0 + 1);
		      rc01 = MAPIN(inputMapC, ix0, iy0 + 1);
		    }
		  else
		    {
		      rx01 = 0.0;
		  ...
		  if (ix0 < mpwi)
		    {
		      rx10 = MAPIN(inputMapX, ix0 + 1, iy0);
		      ry10 = MAPIN(inputMapY, ix0 + 1, iy0);
		      rc10 = MAPIN(inputMapC, ix0 + 1, iy0);
		    }
		  else
		    {
		      rx10 = 0.0;
		  ...
		  if (ix0 < mpwi && iy0 < mphi)
		    {
		      rx11 = MAPIN(inputMapX, ix0 + 1, iy0 + 1);
		      ry11 = MAPIN(inputMapY, ix0 + 1, iy0 + 1);
		      rc11 = MAPIN(inputMapC, ix0 + 1, iy0 + 1);
		    }
		  else
		    {
		      rx11 = 0.0;
	      ...
	      x0 = inputMapFactor * rx + (inputMapFactor - 1.0) / 2.0;
	      y0 = inputMapFactor * ry + (inputMapFactor - 1.0) / 2.0;
