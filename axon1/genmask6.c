/* genmask6  simply finds all pixels with value 0 and masks them out;
   this is used in the htem stack to mask out black borders and also
   missing tiles */
            ...
            char **envp)
{
  ...
  for (i = 1; i < argc; ++i)
	...
	strcpy(skipFile, argv[i]);
      ...
      {
	if (++i == argc || sscanf(argv[i], "%d", &(c.threshold)) != 1)
	  {
  ...
  if (error)
    {
      fprintf(stderr, "Usage: genmask6 -input file_prefix -output file_prefix\n");
...
WorkerTask ()
{
  ...
  /* set the result bitmap to make everything valid */
  memset(mask, 0xff, h*bpl);

  for (y = 0; y < h; ++y)
    for (x = 0; x < w; ++x)
      if (image[y*w + x] < c.threshold)
