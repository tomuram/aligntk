...
typedef struct Context {
  ...
  char discontinuityMaskName[PATH_MAX];
  int discontinuityMaskDigits;
            ...
            char **envp)
{
  ...
  for (i = 1; i < argc; ++i)
    ...
    else if (strcmp(argv[i], "-discontinuity_mask") == 0)
      {
	if (++i == argc)
	  {
	    error = 1;
	...
	strcpy(c.discontinuityMaskName, argv[i]);
      }
  ...
  if (error)
    {
      ...
      fprintf(stderr, "              [-discontinuity_mask <mask_prefix>]\n");
  ...
  if (c.discontinuityMaskName[0] != '\0')
    {
      /* check what discontinuity masks are present */
      for (i = strlen(c.discontinuityMaskName)-1;
	   i >= 0 && c.discontinuityMaskName[i] != '/';
	   --i) ;
      if (i >= 0)
	{
	  strncpy(discontinuityMaskDirName, c.discontinuityMaskName, i+1);
	  discontinuityMaskDirName[i+1] = '\0';
	  strcpy(discontinuityMaskPrefix, &c.discontinuityMaskName[i+1]);
	}
      else
	{
	  strcpy(discontinuityMaskDirName, "./");
	  strcpy(discontinuityMaskPrefix, c.discontinuityMaskName);
	}
      
      /* read the directory to look for files of the form nameNNNN.pbm */
      dir = opendir(discontinuityMaskDirName);
      if (dir == NULL)
	Error("Could not open directory %s for discontinuity mask files\n");
      discontinuityMaskPrefixLen = strlen(discontinuityMaskPrefix);
      discontinuityMaskMinN = 1000000000;
      discontinuityMaskMaxN = -1;
      c.discontinuityMaskDigits = -1;
      while ((de = readdir(dir)) != NULL)
	{
	  if (strncmp(de->d_name, discontinuityMaskPrefix, discontinuityMaskPrefixLen) == 0 &&
	      (len = strlen(de->d_name)) > 4 &&
	      strcmp(&(de->d_name[len-4]), ".pbm") == 0)
	    {
	      if (sscanf(&(de->d_name[discontinuityMaskPrefixLen]), "%d%n",
			 &n, &digitLen) < 1)
		continue;
	      if (c.discontinuityMaskDigits < 0)
		c.discontinuityMaskDigits = digitLen;
	      else if (digitLen != c.discontinuityMaskDigits)
		Error("Inconsistent number of digits in discontinuity mask file names: %s\n", de->d_name);
	      if (n < discontinuityMaskMinN)
		{
		  discontinuityMaskMinN = n;
		  
		  /* read the header to determine image size */
		  sprintf(fn, "%s%s", discontinuityMaskDirName, de->d_name);
		  f = fopen(fn, "r");
		  if (f == NULL)
		    Error("Could not open discontinuity mask file %s", fn);
		  if (!ReadHeader(f, &tc, &iw, &ih, &m))
		    {
		      fclose(f);
		      Error("Invalid image file header: %s\n", fn);
		  ...
		  if (tc != '4')
		    {
		      fclose(f);
		      Error("Bad .pbm header in discontinuity mask file: %s\n", fn);
		    }
		  fclose(f);
		}
	      if (n > discontinuityMaskMaxN)
		discontinuityMaskMaxN = n;
	    }
	}
      closedir(dir);
      if (discontinuityMaskMaxN < 0)
	Error("No discontinuity mask image files found.\n");
      if (discontinuityMaskMaxN < minN || discontinuityMaskMinN > maxN)
	Error("No corresponding discontinuity mask image files found.\n");
    }
  else
    {
      c.discontinuityMaskDigits = -1;
...
WorkerTask ()
{
  ...
  if (!update && c.discontinuityMaskName[0] != '\0')
    {
      sprintf(fn, "%s%0.*d.pbm", c.discontinuityMaskName, c.discontinuityMaskDigits, t.imageSlice);
      if (stat(fn, &sb) == 0 && (double) (sb.st_mtime) > c.afterTime)
	update = 1;
    }
