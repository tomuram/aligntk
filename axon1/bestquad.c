...
main (int argc, char **argv)
{
  ...
  outputLevel = 8;
  if ((argc != 3 && argc != 4) ||
      argc == 4 && sscanf(argv[3], "%d", &outputLevel) != 1)
    {
      fprintf(stderr, "Usage: bestquad input.map output.map output_level\n");
      exit(1);
    }
  ...
  /* now determine best quadratic transformation */
  ...
  rLevel = outputLevel;
  rFactor = 1 << rLevel;
  nx = (xMax + rFactor - 1) / rFactor;
  ny = (yMax + rFactor - 1) / rFactor;
  rmap = (MapElement *) malloc(nx * ny * sizeof(MapElement));
  printf("nx = %d ny = %d\n", nx, ny);
  ...
  for (y = 0; y < ny; ++y)
	...
	xv = x * rFactor;
	yv = y * rFactor;
	rmap[y*nx+x].x = (a[4] * xv * xv +
			  a[5] * yv * yv +
			  a[3] * xv * yv +
			  a[1] * xv +
			  a[2] * yv +
			  a[0]) / rFactor;
	rmap[y*nx+x].y = (b[4] * xv * xv +
			  b[5] * yv * yv +
			  b[3] * xv * yv +
			  b[1] * xv +
			  b[2] * yv +
  ...
  /* write new map out */
  if (!WriteMap(argv[2], rmap, rLevel, nx, ny, 0, 0,
		imgName, refName,
		UncompressedMap, msg))
    {
