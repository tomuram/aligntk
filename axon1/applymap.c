...
#define PIMAGE(i,x,y)	(x >= 0 && x < w && y >= 0 && y < h ? &i[3*((y)*w + (x))] : &colorBackground[0])
...
main (int argc, char **argv, char **envp)
{
  ...
  if (strlen(inputName) > 4 &&
      (strcmp(&inputName[strlen(inputName) - 4], ".pgm") == 0 ||
       strcmp(&inputName[strlen(inputName) - 4], ".ppm") == 0))
    {
      f = fopen(inputName, "r");
      if (f == NULL)
	...
	Error("Invalid image file header: %s\n", inputName);
      if (tc != '5' && tc != '6')
	Error("Expected binary pgm or ppm file: %s\n", inputName);
      color = (tc == '6');
      if (m != 255 && m != 65535)
	Error("Expected 8-bit or 16-bit pgm or ppm file: %s\n", inputName);
      if (m == 255)
	inputBits = 8;
      else
	inputBits = 16;
      ...
      else
	{
      ...
      /* read the directory to look for files of the form nameNNNN.pgm  (or .ppm) */
      ...
      while ((de = readdir(dir)) != NULL)
	{
	  if (strncmp(de->d_name, inputPrefix, inputPrefixLen) == 0 &&
	      ...
	      if (n < minN)
		{
		  ...
		  if (!ReadHeader(f, &tc, &inputWidth, &inputHeight, &m))
		    Error("Invalid image file header: %s\n", de->d_name);
		  if ((tc != '5' && tc != '6') || (m != 255 && m != 65535))
		    Error("Expected 8-bit or 16-bit pgm file: %s\n", de->d_name);
		  if (color < 0)
		    color = (tc == '6');
		  else if (color ^ (tc == '5'))
		    Error("Image files must either be all pgm or ppm, not mixed.\n");
		  if (m == 255)
		    inputBits = 8;
		  else
		    inputBits = 16;
  ...
  if (forward && noCrop)
      ...
      for (imageSlice = minN; imageSlice <= maxN; ++imageSlice)
	{
	  ...
	  if (strlen(inputName) > 4 &&
	    ...
	    sprintf(fn, "%s%0.*d.%s", inputName, inputDigits, imageSlice,
		    color ? "ppm" : "pgm");
	  f = fopen(fn, "r");
	  if (f == NULL)
  ...
  for (imageSlice = minN; imageSlice <= maxN; ++imageSlice)
	...
	Error("Could not read header of %s\n", fn);
      nPixels = ((size_t) w) * h;
      nChannels = color ? 3 : 1;
      input = (unsigned char *) malloc(nChannels*nPixels*inputBits/8);
      if (fread(input, 1, nChannels*nPixels*inputBits/8, f) !=
	  nChannels*nPixels*inputBits/8)
	Error("Image file %s apparently truncated\n", fn);
      fclose(f);
      image = (float *) malloc(nChannels * w * h * sizeof(float));
      nValues = nChannels * nPixels;
      if (inputBits == 8)
	...
	for (x = -1; x <= mpw; ++x)
      ...
      output = (float *) malloc(nChannels * oWidth * oHeight * sizeof(float));
      ...
      if (forward)
	{
	  ...
	  for (y = oMinY; y <= oMaxY; ++y)
		...
		if (!GridTest(&gridSet, pt))
		  {
		    if (color)
		      memset(&output[3 * ((y - oMinY) * oWidth + (x - oMinX))],
			     0, 3*sizeof(float));
		    else
		...
		if (color)
		  {
		    ptr00 = PIMAGE(image, irx, iry);
		    ptr01 = PIMAGE(image, irx, iry + 1);
		    ptr10 = PIMAGE(image, irx + 1, iry);
		    ptr11 = PIMAGE(image, irx + 1, iry + 1);

		    redValue = (*ptr00) * (rrx - 1.0) * (rry - 1.0)
		      - (*ptr10) * rrx * (rry - 1.0) 
		      - (*ptr01) * (rrx - 1.0) * rry
		      + (*ptr11) * rrx * rry;
		    ...
		    greenValue = (*ptr00) * (rrx - 1.0) * (rry - 1.0)
		      - (*ptr10) * rrx * (rry - 1.0) 
		      - (*ptr01) * (rrx - 1.0) * rry
		      + (*ptr11) * rrx * rry;
		    ...
		    blueValue = (*ptr00) * (rrx - 1.0) * (rry - 1.0)
		      - (*ptr10) * rrx * (rry - 1.0) 
		      - (*ptr01) * (rrx - 1.0) * rry
		      + (*ptr11) * rrx * rry;

		    ptr = &output[3 *((y - oMinY) * oWidth + (x - oMinX))];
		    *ptr++ = redValue;
		    *ptr++ = greenValue;
		    *ptr = blueValue;
		  }
	...
	{
	  /* reverse map  -- this is more straightforward */
      ...
      warpout = (unsigned char*) malloc(nChannels * oPixels * inputBits / 8);
      if (strlen(outputName) > 4 &&
	  (strcmp(&outputName[strlen(outputName) - 4], ".pgm") == 0 ||
	   strcmp(&outputName[strlen(outputName) - 4], ".ppm") == 0))
	strcpy(fn, outputName);
      else
	sprintf(fn, "%s%0.*d.%s", outputName, inputDigits, imageSlice,
		color ? "ppm" : "pgm");
      f = fopen(fn, "w");
      if (f == NULL)
	...
	for (ii = 0; ii < nValues; ++ii)
      ...
      if (fwrite(warpout, inputBits / 8, nValues, f) != nValues)
