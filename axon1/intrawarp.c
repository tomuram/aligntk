...
Compute (char *outputName)
{
  ...
  for (level = startLevel; level >= c.outputLevel; --level)
	...
	dFactor = 10.0;
      //      dFactor = sqrt(((double) (1 << nLevels)) / (1 << level) / 64.0);
	    ...
	    else
 {
      ...
      for (i = 0; i < nCpts; ++i)
	{
      ...
      e = distortion * c.distortion * dFactor - correlation + correspondence * c.correspondence;
      Log("Total energy of level %d map is %f (dist %f - correl %f + corresp %f)\n",
	  level, e, distortion * c.distortion * dFactor, correlation,
      ...
      while (actualCoverage < goalCoverage)
	{
	  ...
	  newE = newDistortion * c.distortion * dFactor - newCorrelation +
  ...
  fprintf(f, "%f %f %f %f\n",
	  correlation
	  - c.distortion * distortion * dFactor
