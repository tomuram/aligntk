/* 
 * first.c  - find image rotation, scale, and translation
 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
            ...
            char **envp)
{
  ...
  if (error)
	...
	fprintf(stderr, "Incomplete option: %s\n", argv[i-1]);
      ...
      fprintf(stderr, "Usage: fftr -input <file_prefix>\n");
 ...
 Log("MASTER setting context\n");
  ...
  if (summaryName[0] != '\0')
	...
	Error("Could not open summary output file %s\n", summaryName);
      ...
      fprintf(f, "         IMAGE     REFERENCE   QUALITY     ROTATION    SCALE       TX          TY\n");
...
WorkerTask ()
{
  ...
  char outputName[PATH_MAX], scoreName[PATH_MAX];
  double sum;
  float mean[2];
  int th;
  ...
  if (c.outputBasename[0] != '\0')
    sprintf(outputName, "%s%s.map", c.outputBasename, t.pair.pairName);
  else
    outputName[0] = '\0';
  ...
  if (imageMaskName[0] != '\0')
	...
	{
	  fprintf(stderr, "image mask %s has different dimensions (%d x %d) than image %s (%d x %d)\n",
  ...
  if (refMaskName[0] != '\0')
	...
	{
	  fprintf(stderr, "reference mask %s has different dimensions than reference %s\n",
		  refMaskName, refName);
	  exit(1);
	}
      ...
      for (it = 0; it < n; ++it)
	{
	  ...
	  for (ir = 0; ir < n; ++ir)
	      ...
	      for (res = nRes - 1; res >= 0; --res)
		{
	      ...
	      if (res < 0)
		{
		  fprintf(stderr, "Could not find point in any resolution level: %d %d %f %f  %f %f %f\n", it, ir, xv, yv, resMin[0], resMin[1], resMin[2]);
		  exit(1);
		}
#if 0
	      hpfx = cos(xv * M_PI / n);
 ...
 findTranslation:
  ...
  for (i = 0; i < nCandidates; ++i)
    {
      ...
      /* if rotation is very close to 0.0, make it exactly 0.0 */
      rotation = -candidates[i].x * M_PI / n;
      if (fabs(rotation) < 0.001 || fabs(rotation - M_PI) < 0.001)
	rotation = 0.0;
	...
	s = scale = 1.0;
	  ...
	  i, rotation * 180.0 / M_PI, scale, smaller, larger);

      offset_x = (n - eiw[larger]) >> 1;
      offset_y = (n - eih[larger]) >> 1;

      if (rFactor == 1 && scale == 1.0 && rotation == 0.0)
	for (y = 0; y < n; ++y)
	  for (x = 0; x < n; ++x)
	    {
		...
		for (dx = 0; dx < sFactor; ++dx)
		  {
		    ...
		    xvp = s * (ct * xv - st * yv) + n_over_2 - offset_x;
		    yvp = s * (st * xv + ct * yv) + n_over_2 - offset_y;
      ...
      if (c.outputImages && i == 0)
	{
      ...
      for (th = 0; th <= 180; th += 180, rotation += M_PI)
	{
	  ...
	  /* generate a picture */
	  if (c.outputImages && i == 0 && radius == 0.0)
	    {
	      v_max = 0.0;
	      v_min = 0.0;
	      for (y = 0; y < n; ++y)
	  ...
	  for (j = 0; j < n2; ++j)
		...
		tx = rFactor * (float) (ix - n);
	      if (iy <= n_over_2)
		ty = rFactor * (float) iy;
	      else
		ty = rFactor * (float) (iy - n);
	      if (larger == 1)
		{
	      ...
	      /* check if in valid region */
	      if (tx < c.minTX * iw[0] * 0.01 || tx > c.maxTX * iw[0] * 0.01 ||
		  ty < c.minTY * ih[0] * 0.01 || ty > c.maxTY * ih[0] * 0.01)
		{
	      ...
	      for (k = 0; k < nFinalCandidates; ++k)
		{
		  ...
		  if (quality > finalCandidates[k].quality)
		    break;
		  if (fabs(finalCandidates[k].tx - tx) < c.minTranslationalSeparation * n &&
		      fabs(finalCandidates[k].ty - ty) < c.minTranslationalSeparation * n)
		    goto nextFinalPosition;
		}
	      ...
	      while (k < nFinalCandidates)
		{
		  if (finalCandidates[k].rotation != rotation ||
		      finalCandidates[k].scale != scale ||
		      fabs(finalCandidates[k].tx - tx) >= c.minTranslationalSeparation * n ||
		      fabs(finalCandidates[k].ty - ty) >= c.minTranslationalSeparation * n)
		    {
		      ++k;
		      continue;
  ...
  if (outputName[0] != '\0')
	...
	{
	  yv = ((float) (y * md - iMinY)) / rFactor +
	    offset_y - n_over_2;
	  for (x = mMinX; x <= mMaxX; ++x)
	    {
	      xv = ((float) (x * md - iMinX)) / rFactor +
		offset_x - n_over_2;
	      xvp = scale * (ct * xv - st * yv) + tx / rFactor + n_over_2;
	      yvp = scale * (st * xv + ct * yv) + ty / rFactor + n_over_2;
	      i = (y - mMinY) * mw + (x - mMinX);
	      map[i].x = (xvp - offset2_x) * rFactor / md;
	      map[i].y = (yvp - offset2_y) * rFactor / md;
	      map[i].c = 1.0;
      ...
      fwrite(map, mh*mw*sizeof(MapElement), 1, f);
      fclose(f);
    }
  ...
  if (c.outputBasename[0] != '\0')
    {
      sprintf(scoreName, "%s%s.score", c.outputBasename, t.pair.pairName);
      f = fopen(scoreName, "w");
      fprintf(f, "%f %f %f %f %f\n",
