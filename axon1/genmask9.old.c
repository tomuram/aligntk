...
#define FILL_FROM_BOUNDARY_METHOD	1
#define FILL_BLACK_FROM_BOUNDARY_METHOD	2
#define SELECT_NONBLACK_CENTER_METHOD	3
...
WorkerTask ()
{
  ...
  switch (c.maskMethod)
      ...
      for (x = 0; x < imageWidth; ++x)
	{
	  ...
	  if (image[y*w + x] == 0 &&
	      (mask[y*bpl + (x >> 3)] & (0x80 >> (x & 7))) != 0)
	    FloodFill(image, mask, w, h, x, y, Black);
	  y = imageHeight-1;
	  if (image[y*w + x] == 0 &&
	      (mask[y*bpl + (x >> 3)] & (0x80 >> (x & 7))) != 0)
	    FloodFill(image, mask, w, h, x, y, Black);
    ...
    case SELECT_NONBLACK_CENTER_METHOD:
      ...
      if (image[(h/2)*w + (w/2)] != 0)
	FloodFill(image, mask, w, h, w/2, h/2, NotBlack);
      else if (image[(h/2)*w + (w/2-1)] != 0)
	FloodFill(image, mask, w, h, w/2-1, h/2, NotBlack);
      else if (image[(h/2-1)*w + (w/2)] != 0)
	FloodFill(image, mask, w, h, w/2, h/2-1, NotBlack);
      else if (image[(h/2-1)*w + (w/2-1)] != 0)
	FloodFill(image, mask, w, h, w/2-1, h/2-1, NotBlack);
      else
	Error("All four sampled center pixels of image %s are black.\n",
	      fn);
      for (y = 0; y < h; ++y)
	for (x = 0; x < w; ++x)
