...
WorkerTask ()
{
  ...
  if (c.outputPairwiseMaskBasename[0] != '\0')
    sprintf(outputMaskName, "%s%s.pbm", c.outputPairwiseMaskBasename,
	    t.pair.pairName);
  else if (c.outputMaskBasename[0] != '\0')
    sprintf(outputMaskName, "%s%s.pbm", c.outputMaskBasename,
	    t.pair.imageName[0]);
  ...
  for (imi = 0; imi < 2; ++imi)
      ...
      if (maskName[imi][0] != '\0')
	{
	  ...
	  if (!ReadBitmap(maskName[imi], &masks[imi][0],
			  ...
			  r.message))
	    {
	      sprintf(r.message, "Could not read mask %s\n", maskName[imi]);
	      return;
	    }
  ...
  if (outputMaskName[0] != '\0')
		      ...
		      r.message))
	{
	  ...
	  sprintf(r.message, "Could not read output mask %s\n", outputMaskName);
	  return;
	}
	  ...
	  omh != imageHeight[0][0])
	{
	  ...
	  sprintf(r.message, "Output mask %s size does not match image size\n",
		  outputMaskName);
	  return;
	}
...
Compute (char *outputName, char *outputWarpedName, char *outputCorrelationName)
{
  ...
  for (level = startLevel; level >= c.outputLevel; --level)
	...
	if (inputMapFactor != 0 && nCpts < 2)
	  {
	      ...
	      for (x = 0; x < mpw; ++x)
		{
		  ...
		  Log("Setting map[%d,%d] to (%f %f)\n",
		      x, y, rx / lFactor, ry / lFactor);
		}
	  }
	else if (nCpts >= 2)
	  {
	    /* use an affine or quadratic map based on the given
	...
	else if (nCpts == 1)
	  {
	    ...
	    for (y = 0; y < mph; ++y)
	      for (x = 0; x < mpw; ++x)
		SETMAP(map, mpw, x, y,
		       ((float) (x + mox)) + (cpts[0].ix - cpts[0].rx) / lFactor,
		       ((float) (y + moy)) + (cpts[0].iy - cpts[0].ry) / lFactor,
		       1.0);
	    Log("Setting initial map to a translation of (%f, %f)\n",
		cpts[0].ix - cpts[0].rx, cpts[0].iy - cpts[0].ry);
	  }
	else
	  {
      ...
      if (nValid == 0)
	{
      ...
      {
	MapElement *submap = (MapElement *) malloc(36 * sizeof(MapElement));
	sprintf(fn, "mmaps/%d.%0.4d.map", level, mmi);
	for (y = 0; y < 6; ++y)
	  for (x = 0; x < 6; ++x)
	    {
	      GETMAP(map, mpw, x + ichx0, y + ichy0, &rx00, &ry00, &rc00);
	      SETMAP(submap, 6, x, y, rx00, ry00, rc00);
	    }
	if (!WriteMap(fn, submap, level, 6, 6,
		      mox + ichx0, moy + ichy0,
		      t.pair.imageName[0], t.pair.imageName[1],
		      UncompressedMap, msg))
	  Error("Could not write movie map %s :\n%s\n",
		fn, msg);
	free(submap);
	++mmi;
      }
      ...
      while (moveCount < goalMoveCount)
	{
	  ...
	  if (accept)
		...
		cpts[i].energy = cpts[i].newEnergy;
	      ...
	      // write out the map if the target area is affected
	      if (ichx1 >= changeMinX && ichx0 <= changeMaxX &&
		  ichy1 >= changeMinY && ichy0 <= changeMaxY)
		{
		  MapElement *submap = (MapElement *) malloc(36 * sizeof(MapElement));
		  sprintf(fn, "mmaps/%d.%0.4d.map", level, mmi);
		  for (y = 0; y < 6; ++y)
...
GetTimestamp (char *timestamp, size_t size)
{
  ...
  if (size - len > 8)
    sprintf(&timestamp[len], ".%0.6d", current_time.tv_usec);
  return(timestamp);
}
